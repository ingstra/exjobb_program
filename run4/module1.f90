...
&channels, rho_im_filename, rho_re_filename)
    ...
    integer :: k, p, q,maxlik_count, NFock=2, Nbins=100, nangles=10
...
!!$  function envelope2(gamma,Omega,t_start,t_end,t,channels) result(y)
!!$    real(dp), intent(in) :: gamma, t_end, t_start, t,Omega
!!$    integer, intent(in) :: channels
!!$    real(dp) :: N,y,const,gamma1,term1,term2,term3,lambdaplus,lambdaminus, kappa,os,gs,t
...
!!$    two_os = 2._dp*os
!!$    gs = gamma**2
!!$    kappa = sqrt((gamma/4)**2-os)
!!$    lambdaminus = (-gamma*(10._dp*os-gs)-(gs-two_os))/(gs+two_os)/4._dp
!!$    lambdaplus = (-gamma*(10._dp*os-gs)+(gs-two_os))/(gs+two_os)/4._dp
!!$    N=os/(gs+two_os)
!!$    term1 = gs*exp(-i*t)/(gs+two_os)
!!$    term2 = exp(-(gamma/2+i)*t)
!!$    term3 = lambdaplus*exp(-(3._dp*gamma/4 -kappa)+i)*t)
!!$    term4 = lambdaminus*exp(-(3._dp*gamma/4 +kappa)+i)*t
!!$
!!$    y= N*(term1+term2-term2+term4)
!!$
!!$
!!$  end function envelope2
  ...
  subroutine correlations(nruns, c, cdagger, dt ,filename,t_start, rhovec_zero, H)
    ...
    do j=1,nruns
       ...
       if (j*dt .gt. t_start) then
          ...
          tmpvec = matmul(kronecker(conjg(c),c,2),rhovec)
...
& 2._dp*const*trace(term2)*(trace(term2)*rho-term2))*(dW**2._dp-dt)
  ...
  function liouvillian(c, H_eff) result(y)
    ...
    y = -i*kronecker(identity,H_eff,2) + i*kronecker(conjg(H_eff),identity,2) +&
         &kronecker(conjg(c),c,2)

  end function liouvillian

  function kronecker(A,B,s) result(y)
    integer, intent(in) :: s
